...
class App extends Component {
  ...
  onClickVisitBtn = id => {
    ...
    const updated = countriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        return {...eachCountry, isVisited: !eachCountry.isVisited}
      }
      return eachCountry
    })
    this.setState({countriesList: updated})
  }

  onRemoveCountryItem = id => {
    const {countriesList} = this.state

    const filteredData = countriesList.map(eachCountry => {
      if (eachCountry.id === id) {
        return {...eachCountry, isVisited: !eachCountry.isVisited}
      }
      return eachCountry
    })
    this.setState({countriesList: filteredData})
  }
          ...
          <ul className="visited-countries">
            {visitedCountries.map(eachItem => (
              <VisitedCountries
                ...
                onRemoveCountryItem={this.onRemoveCountryItem}
              />
            ))}
          </ul>
        )}
